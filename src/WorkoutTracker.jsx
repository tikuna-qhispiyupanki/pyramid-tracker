...
import { Activity, TrendingUp, Calendar, Dumbbell, Plus, Trash2, Download, Upload, BarChart3, Flame, Target, Database, Trophy, Zap, Award, Timer, Play, Pause, RotateCcw, Save, X, Info, Mail, Check, LogOut, Cloud } from 'lucide-react';
import { supabase, sessionsAPI } from './supabaseClient';
import LoginScreen from './LoginScreen';

// localStorage shim for legacy non-session preferences (stopwatch state, last finishers, etc.)
// User data (sessions) goes to Supabase, not localStorage
...
export default function WorkoutTracker() {
  ...
  const [legacyLocalSessions, setLegacyLocalSessions] = useState([]);
  ...
  useEffect(() => {
    ...
    const loadSessions = async () => {
      setLoading(true);
      try {
        const sessions = await sessionsAPI.list(user.id);
        setSessions(sessions);

        // Check if there are legacy localStorage sessions to migrate
        if (sessions.length === 0) {
          try {
            const legacy = localStorage.getItem('pyramid:sessions');
            if (legacy) {
              const parsed = JSON.parse(legacy);
              if (Array.isArray(parsed) && parsed.length > 0) {
                setLegacyLocalSessions(parsed);
  ...
  const loadSeed = async () => {
    if (!user) return;
    try {
      const created = await sessionsAPI.bulkInsert(SEED_DATA, user.id);
      setSessions(prev => [...prev, ...created].sort((a, b) => new Date(a.date) - new Date(b.date)));
      setShowSeedPrompt(false);
    } catch (err) {
      alert('Erreur lors de l\'import du seed : ' + err.message);
    }
  };
  ...
  const migrateLegacySessions = async () => {
    if (!user || legacyLocalSessions.length === 0) return;
    try {
      // Strip ids from legacy sessions (they'll get new UUIDs from Supabase)
      const toMigrate = legacyLocalSessions.map(s => {
        const { id, ...rest } = s;
        return rest;
      });
      const created = await sessionsAPI.bulkInsert(toMigrate, user.id);
      setSessions(created.sort((a, b) => new Date(a.date) - new Date(b.date)));
      // Clear legacy
      localStorage.removeItem('pyramid:sessions');
      setShowMigrationPrompt(false);
      setLegacyLocalSessions([]);
      alert(`${created.length} séances importées avec succès depuis ton ancien stockage local.`);
    } catch (err) {
  ...
  const addSession = async (session) => {
    if (!user) return;
    try {
      const created = await sessionsAPI.create(session, user.id);
      setSessions(prev => [...prev, created].sort((a, b) => new Date(a.date) - new Date(b.date)));
      setShowForm(false);
      setEditingId(null);
    } catch (err) {
  ...
  const updateSession = async (id, updated) => {
    if (!user) return;
    try {
      const result = await sessionsAPI.update(id, updated, user.id);
      setSessions(prev => prev.map(s => s.id === id ? result : s).sort((a, b) => new Date(a.date) - new Date(b.date)));
      setShowForm(false);
      setEditingId(null);
    } catch (err) {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const merge = sessions.length > 0 && confirm(
          `Tu as déjà ${sessions.length} séance${sessions.length > 1 ? 's' : ''} dans ton compte.\n\n` +
          `OK = Fusionner avec les ${parsed.length} séances importées (les nouvelles s'ajoutent)\n` +
          `Annuler = Remplacer entièrement par les ${parsed.length} séances importées`
        );

        // Strip ids from imported sessions (Supabase will generate new UUIDs)
        const toImport = parsed.map(s => {
          const { id, createdAt, updatedAt, ...rest } = s;
          return rest;
        });

        if (!merge && sessions.length > 0) {
          // Replace mode: delete all existing first
          if (!confirm('CONFIRME : effacer toutes tes séances actuelles ?')) return;
          await sessionsAPI.deleteAll(user.id);
        }

        // Insert imported sessions
        const created = await sessionsAPI.bulkInsert(toImport, user.id);
        ...
        alert(`Import réussi : ${created.length} séances ajoutées.`);
      } catch (err) {
  ...
  const resetAll = async () => {
    ...
    if (!confirm('Vraiment sûr ? Tape OK dans la fenêtre suivante pour confirmer.')) return;
    try {
      await sessionsAPI.deleteAll(user.id);
      setSessions([]);
      setShowSeedPrompt(true);
  ...
  // Auth flow: show login if not authenticated
  if (authChecking) {
    ...
    <div style={styles.app}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight} data-scroll-row>
          ...
          <button style={styles.iconBtn} onClick={handleSignOut} title="Déconnexion">
            <LogOut size={14} />
          </button>
      ...
      {showMigrationPrompt && (
        <div style={styles.modal} onClick={(e) => e.target === e.currentTarget && skipMigration()}>
          <div style={{ ...styles.aboutModal, maxWidth: 440 }}>
            <div style={styles.aboutHeader}>
              <button style={styles.iconBtn} onClick={skipMigration} aria-label="Fermer"><X size={18} /></button>
            </div>
            <div style={{ ...styles.aboutContent, paddingTop: 0 }}>
              <div style={{ ...styles.aboutLogo, background: '#3da9d9' }}>
                <Cloud size={32} strokeWidth={2.5} style={{ color: '#0a0a0a' }} />
              </div>
              <h1 style={{ ...styles.aboutTitle, fontSize: 28 }}>Migration possible</h1>
              <p style={{ ...styles.aboutText, marginTop: 16 }}>
                On a détecté <strong style={{ color: '#3da9d9' }}>{legacyLocalSessions.length} séance{legacyLocalSessions.length > 1 ? 's' : ''}</strong> dans ton ancien stockage local de cet appareil.
              </p>
              <p style={styles.aboutText}>
                Veux-tu les importer dans ton compte cloud ? Elles seront disponibles sur tous tes appareils après.
              </p>
              <button
                style={{ ...styles.submitBtnCloud, marginTop: 16 }}
                onClick={migrateLegacySessions}
              >
                <Cloud size={16} /> Importer dans mon compte
              </button>
              <button
                style={{ ...styles.secondaryBtn, width: '100%', marginTop: 8 }}
                onClick={skipMigration}
              >
                Ignorer (garder local)
              </button>
            </div>
          </div>
        </div>
      ...
      <footer style={styles.footer}>
        <span>Mannek · 2026 · {user?.email}</span>
      </footer>
    </div>
  );
}
...
const styles = {
  ...
  submitBtnCloud: { width: '100%', background: '#3da9d9', color: '#0a0a0a', border: 'none', padding: 14, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', letterSpacing: 2, borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 },

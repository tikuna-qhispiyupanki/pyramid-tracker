...
// =============== STOPWATCH ===============
function Stopwatch({ onClose, onSave, suggestedCode, setBCountSoFar = 0 }) {
  ...
  const [plankSetIdx, setPlankSetIdx] = useState(0); // 0 → 2 (3 sets)
  ...
  const [plankResults, setPlankResults] = useState([]); // [{ duration, completed }]
  ...
  const handleNextLevel = () => {
    ...
    if (currentLevel + 1 >= totalLevels) {
      // Pyramid finished
      if (setType === 'B') {
        // Set B → ask if user wants progression first, then go to plank phase
        ...
        setPlankDuration(recommendedDur >= plankDuration ? plankDuration : plankDuration);
        // Default initial duration based on count if user has never set it
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        const newResults = [...plankResults, { duration: plankDuration, completed: true }];
        ...
        if (plankSetIdx + 1 >= 3) {
          // All 3 plank sets done, go to summary
          setPhase('finished');
          setShowSummary(true);
        } else {
    ...
    return () => clearInterval(interval);
  }, [plankRunning, plankStartTime, plankDuration, plankSetIdx, plankResults]);
  ...
  const skipPlankSet = () => {
    const newResults = [...plankResults, { duration: plankDuration, completed: false }];
    setPlankResults(newResults);
    setPlankRunning(false);
    if (plankSetIdx + 1 >= 3) {
      setPhase('finished');
          ...
          {setType === 'B' && plankResults.length > 0 && (
            <div style={{ ...styles.summarySplitsTable, marginTop: 12 }}>
              ...
              {plankResults.map((r, i) => (
                <div key={i} style={styles.summarySplitsRow}>
                  <span style={{ color: '#3da9d9', fontWeight: 700 }}>Série {i + 1}</span>
        ...
        <div style={styles.stopwatchRunning}>
          ...
          <div style={styles.swTop}>
            <div style={{ fontSize: 10, color: '#666', textTransform: 'uppercase', letterSpacing: 2, fontFamily: "'JetBrains Mono', monospace" }}>
              Pyramide terminée · Finisher Planche
            </div>
          ...
          {/* Progression prompt for Set B count milestone */}
          {showProgressionPrompt && plankDuration < recommendedDur && (
            <div style={{ ...styles.compareHint, background: 'rgba(61,169,217,0.1)', borderColor: 'rgba(61,169,217,0.3)' }}>
              ...
              <span>
                <strong>Tu as fait {setBCountSoFar + 1} Set B.</strong> L'app suggère de passer la planche à <strong>{recommendedDur}s</strong>. Prêt ?
              </span>
              <button
                style={{ ...styles.iconBtnSmall, marginLeft: 'auto', borderColor: setColorBlue, color: setColorBlue }}
                onClick={() => { setPlankDuration(recommendedDur); setShowProgressionPrompt(false); setPlankTimeLeft(recommendedDur); }}
              >Oui</button>
              <button
                style={styles.iconBtnSmall}
                onClick={() => setShowProgressionPrompt(false)}
              >Non</button>
            </div>
          )}
            ...
            <div style={styles.swLastSplits}>
              ...
              {plankResults.map((r, i) => (
                <div key={i} style={styles.swSplitRow}>
                  <span style={{ color: '#888' }}>Série {i + 1}</span>
          ...
          {plankRunning ? (
            <button
              ...
              onClick={() => {
                // Stop the timer early - record as completed with elapsed time
                const actualDuration = plankDuration - plankTimeLeft;
                const newResults = [...plankResults, { duration: actualDuration, completed: actualDuration >= plankDuration * 0.9 }];
                setPlankResults(newResults);
                setPlankRunning(false);
                if (plankSetIdx + 1 >= 3) {
                  setPhase('finished');

...
function Stopwatch({ onClose, onSave, suggestedCode, setBCountSoFar = 0, levelRecords = { A: {}, B: {} } }) {
  ...
  const [plankResults, setPlankResults] = useState([]); // [{ duration, completed, side? }]
  const [showProgressionPrompt, setShowProgressionPrompt] = useState(false);
  const [selectedFinisherId, setSelectedFinisherId] = useState('plank'); // default to planche frontale
  const [showFinisherSelection, setShowFinisherSelection] = useState(false);

  const selectedFinisher = getFinisherById(selectedFinisherId);
  ...
  const handleNextLevel = () => {
    ...
    if (currentLevel + 1 >= totalLevels) {
      ...
      if (setType === 'B') {
        ...
        // Try to load last used finisher
        try {
          const lastFinisher = localStorage.getItem('pyramid:lastFinisher');
          if (lastFinisher && getFinisherById(lastFinisher)) {
            setSelectedFinisherId(lastFinisher);
          }
        } catch (e) {}
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        const side = selectedFinisher.unilateral ? (plankSetIdx % 2 === 0 ? 'G' : 'D') : null;
        const newResults = [...plankResults, { duration: plankDuration, completed: true, side }];
        setPlankResults(newResults);
        if (typeof navigator !== 'undefined' && navigator.vibrate) {
          navigator.vibrate([200, 100, 200]);
        }
        if (plankSetIdx + 1 >= totalPlankSets) {
          setPhase('finished');
          setShowSummary(true);
        } else {
          setPlankSetIdx(plankSetIdx + 1);
          setPlankTimeLeft(plankDuration);
        }
      }
    }, 100);
    return () => clearInterval(interval);
  }, [plankRunning, plankStartTime, plankDuration, plankSetIdx, plankResults, totalPlankSets, selectedFinisher.unilateral]);

  const startPlankSet = () => {
    // Save finisher choice for next time
    try { localStorage.setItem('pyramid:lastFinisher', selectedFinisherId); } catch (e) {}
  ...
  const skipPlankSet = () => {
    const side = selectedFinisher.unilateral ? (plankSetIdx % 2 === 0 ? 'G' : 'D') : null;
    const newResults = [...plankResults, { duration: plankDuration, completed: false, side }];
    setPlankResults(newResults);
    setPlankRunning(false);
    if (plankSetIdx + 1 >= totalPlankSets) {
      setPhase('finished');
  ...
  const handleSaveSession = () => {
    ...
    if (setType === 'B' && plankResults.length > 0) {
      data.plank = {
        ...
        finisherName: selectedFinisher.name,
    ...
    return (
      <div style={styles.stopwatchModal}>
        <div style={styles.stopwatchSummary}>
          ...
          {/* Plank summary for Set B */}
          {setType === 'B' && plankResults.length > 0 && (
            <div style={{ ...styles.summarySplitsTable, marginTop: 12 }}>
              <div style={styles.summarySplitsHeader}>
                <span>{selectedFinisher.short}</span>
                <span>Cible</span>
                <span>Réalisé</span>
                <span>Statut</span>
              </div>
              {plankResults.map((r, i) => (
                <div key={i} style={styles.summarySplitsRow}>
        ...
        <div style={styles.stopwatchModal}>
          <div style={styles.stopwatchSetup}>
            ...
            <p style={{ fontSize: 12, color: '#888', marginBottom: 16, lineHeight: 1.6 }}>
              Pyramide terminée en <strong style={{ color: setColorBlue }}>{formatStopwatch(elapsed).replace(/\.\d$/, '')}</strong>. Choisis ton finisher de gainage.
            </p>
              ...
              {FINISHERS_B.map(fin => (
                <button
                  ...
                  style={{
                    ...
                    borderColor: selectedFinisherId === fin.id ? setColorBlue : '#2a2a2a',
                    background: selectedFinisherId === fin.id ? 'rgba(61,169,217,0.08)' : '#0a0a0a'
                  }}
                  onClick={() => setSelectedFinisherId(fin.id)}
                >
                  <div style={styles.finisherOptionHeader}>
                    <span style={{ ...styles.finisherOptionName, color: selectedFinisherId === fin.id ? setColorBlue : '#fff' }}>
                      {fin.name}
                    </span>
            ...
            <div style={{ marginTop: 20 }}>
              <label style={styles.formLabel}>Durée par série</label>
        ...
        <div style={styles.stopwatchRunning}>
          ...
          {plankRunning ? (
            <button
              ...
              onClick={() => {
                ...
                const side = selectedFinisher.unilateral ? (plankSetIdx % 2 === 0 ? 'G' : 'D') : null;
                const newResults = [...plankResults, { duration: actualDuration, completed: actualDuration >= plankDuration * 0.9, side }];
                setPlankResults(newResults);
                setPlankRunning(false);
                if (plankSetIdx + 1 >= totalPlankSets) {
                  setPhase('finished');
          ...
          <div style={styles.swSecondaryBtns}>
            <button style={styles.swSecondaryBtn} onClick={skipPlankSet} disabled={plankRunning}>
              <X size={14} />
              {isLastSet ? 'Passer (terminer)' : 'Passer cette série'}
            </button>
  ...
  return (
    <div style={styles.stopwatchModal}>
      <div style={{ ...styles.stopwatchRunning, border: paceColor ? `2px solid ${paceColor}` : 'none', borderRadius: paceColor ? 8 : 0, transition: 'border-color 0.5s ease' }}>
        {/* Top: meta */}
        <div style={styles.swTop}>
          <div style={{ fontSize: 10, color: '#666', textTransform: 'uppercase', letterSpacing: 2, fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            {currentExercises.map((exo, i) => {
              ...
              return (
                <span key={i} style={styles.swExerciseBadge}>
                  <span style={{ color: setColor, fontWeight: 700, marginRight: 4 }}>{i + 1}</span>
                  <span>{exo.name}</span>
                  <strong style={{ marginLeft: 6, color: '#fff' }}>{reps} {unitPlural}</strong>
                </span>
              );
            })}
          </div>
